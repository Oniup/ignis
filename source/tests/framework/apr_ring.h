...
#define APR_RING_CONCAT(h1, h2, elem, link)                                                       \
  do {                                                                                            \
    if (!APR_RING_EMPTY((h2), elem, link)) {                                                      \
      APR_RING_SPLICE_BEFORE(APR_RING_SENTINEL((h1), elem, link), APR_RING_FIRST((h2)),           \
...
#define APR_RING_PREPEND(h1, h2, elem, link)                                                      \
  do {                                                                                            \
    if (!APR_RING_EMPTY((h2), elem, link)) {                                                      \
      APR_RING_SPLICE_AFTER(APR_RING_SENTINEL((h1), elem, link), APR_RING_FIRST((h2)),            \
                            APR_RING_LAST((h2)), link);                                           \
      APR_RING_INIT((h2), elem, link);                                                            \
    }                                                                                             \
  } while (0)
...
#define APR_RING_FOREACH(ep, head, elem, link)                                                    \
  for (ep = APR_RING_FIRST(head); ep != APR_RING_SENTINEL(head, elem, link);                      \
...
#define APR_RING_FOREACH_SAFE(ep1, ep2, head, elem, link)                                         \
  for (ep1 = APR_RING_FIRST(head), ep2 = APR_RING_NEXT(ep1, link);                                \
       ep1 != APR_RING_SENTINEL(head, elem, link); ep1 = ep2, ep2 = APR_RING_NEXT(ep1, link))
...
#  define APR_RING_CHECK_ONE(msg, ptr) fprintf(stderr, "*** %s %p\n", msg, ptr)
...
#  define APR_RING_CHECK_CONSISTENCY(hp, elem, link)                                              \
    APR_RING_CHECK_ELEM_CONSISTENCY(APR_RING_SENTINEL(hp, elem, link), elem, link)

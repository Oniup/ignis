...
#define FILL_IN_FAILED_REASON(tc, fname, fcname, lineno, reason_fmt, ...)                         \
  do {                                                                                            \
...
void lcut_int_equal(lcut_tc_t* tc, const int expected, const int actual, int lineno,
  ...
  if (expected != actual) {
    FILL_IN_FAILED_REASON(tc, fname, fcname, lineno, "expected<%d> : actual<%d>", expected,
                          actual);
  }
}
...
void lcut_int_nequal(lcut_tc_t* tc, const int expected, const int actual, int lineno,
  ...
  if (expected == actual) {
    FILL_IN_FAILED_REASON(tc, fname, fcname, lineno, "not expected<%d> : actual<%d>", expected,
                          actual);
  }
}
...
void lcut_str_equal(lcut_tc_t* tc, const char* expected, const char* actual, int lineno,
  ...
  FILL_IN_FAILED_REASON(tc, fname, fcname, lineno, "expected<%s> : actual<%s>",
...
void lcut_str_nequal(lcut_tc_t* tc, const char* expected, const char* actual, int lineno,
  ...
  FILL_IN_FAILED_REASON(tc, fname, fcname, lineno, "not expected<%s> : actual<%s>",
                        expected ? expected : "NULL", actual ? actual : "NULL");
}
...
void lcut_assert(lcut_tc_t* tc, const char* msg, int condition, int lineno, const char* fcname,
  ...
  FILL_IN_FAILED_REASON(tc, fname, fcname, lineno, "%s", msg);
}
...
void lcut_true(lcut_tc_t* tc, int condition, int lineno, const char* fcname, const char* fname) {
  ...
  FILL_IN_FAILED_REASON(tc, fname, fcname, lineno, "%s", "");
}
      ...
      APR_RING_FOREACH(tc, &(ts->tc_head), lcut_tc_t, link) {
        if (tc != NULL) {
          ...
          if (tc->status == TEST_CASE_SUCCESS) {
            ...
            ts->failed++;
            printf(FAILURE_TIP_FMT, tc->desc, tc->fcname, tc->line, tc->fname, tc->reason);

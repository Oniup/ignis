...
#define TEST(_name)       static void _name(lcut_tc_t* tc, void* data)
#define TEST_SUITE(_name) void _name(lcut_test_t* _cut_test)

#define SUITE_INIT(_name, _setup, _teardown)                                                      \
    lcut_ts_t* suite       = NULL;                                                                \
    int        _cut_status = 0;                                                                   \
    LCUT_TS_INIT(suite, _name, _setup, _teardown);

#define SUITE_ADD(_function) LCUT_TC_ADD(suite, #_function, _function, NULL, NULL, NULL);
#define PUSH_SUITE()         LCUT_TS_ADD(suite);

#include "framework/apr_ring.h"
#include <stdio.h>
...
#define FAILURE_TIP_FMT                                                                           \
    "\t\t\033[31mCase '%s': Failure occur in %s, %d line in file %s, %s\033[0m\n"
...
enum { TEST_CASE_SUCCESS = 0, TEST_CASE_FAILURE = 1 };
...
int  lcut_test_init(lcut_test_t** test, const char* title, fixture_func setup,
                    fixture_func teardown);
...
int  lcut_tc_add(lcut_ts_t* ts, const char* title, tc_func func, void* para, fixture_func before,
                 fixture_func after);
 ...
 */
#define LCUT_TS_INIT(p, s, setup, teardown)                                                       \
    do {                                                                                          \
 ...
 * e -- extra parameter
 */
#define LCUT_TC_ADD(p, s, f, e, before, after)                                                    \
    do {                                                                                          \
...
void lcut_str_nequal(lcut_tc_t* tc, const char* expected, const char* actual, int lineno,
                     const char* fcname, const char* fname);
void lcut_assert(lcut_tc_t* tc, const char* msg, int condition, int lineno, const char* fcname,
                 const char* fname);
void lcut_true(lcut_tc_t* tc, int condition, int lineno, const char* fcname, const char* fname);

#define LCUT_INT_EQUAL(tc, expected, actual)                                                      \
    do {                                                                                          \
        lcut_int_equal(tc, expected, actual, __LINE__, __FUNCTION__, __FILE__);                   \
    } while (0)

#define LCUT_INT_NEQUAL(tc, expected, actual)                                                     \
    do {                                                                                          \
        lcut_int_nequal(tc, expected, actual, __LINE__, __FUNCTION__, __FILE__);                  \
    } while (0)

#define LCUT_STR_EQUAL(tc, expected, actual)                                                      \
    do {                                                                                          \
        lcut_str_equal(tc, expected, actual, __LINE__, __FUNCTION__, __FILE__);                   \
    } while (0)

#define LCUT_STR_NEQUAL(tc, expected, actual)                                                     \
    do {                                                                                          \
        lcut_str_nequal(tc, expected, actual, __LINE__, __FUNCTION__, __FILE__);                  \
    } while (0)

#define LCUT_ASSERT(tc, msg, condition)                                                           \
    do {                                                                                          \
        lcut_assert(tc, msg, (condition), __LINE__, __FUNCTION__, __FILE__);                      \
    } while (0)

#define LCUT_TRUE(tc, condition)                                                                  \
    do {                                                                                          \
...
void  lcut_mock_obj_return(const char* symbol_name, void* value, const char* fcname, int lineno,
                           const char* fname, int obj_type, int count);
...
#define LCUT_ARG_RETURN(fcname, value)                                                            \
    do {                                                                                          \
        lcut_mock_obj_return(#fcname, (void*)value, __FUNCTION__, __LINE__, __FILE__, MOCK_ARG,   \
                             1);                                                                  \
    } while (0);

#define LCUT_ARG_RETURN_COUNT(fcname, value, count)                                               \
    do {                                                                                          \
        lcut_mock_obj_return(#fcname, (void*)value, __FUNCTION__, __LINE__, __FILE__, MOCK_ARG,   \
                             count);                                                              \
    } while (0);

#define LCUT_RETV_RETURN(fcname, value)                                                           \
    do {                                                                                          \
        lcut_mock_obj_return(#fcname, (void*)value, __FUNCTION__, __LINE__, __FILE__, MOCK_RETV,  \
                             1);                                                                  \
    } while (0);

#define LCUT_RETV_RETURN_COUNT(fcname, value, count)                                              \
    do {                                                                                          \
        lcut_mock_obj_return(#fcname, (void*)value, __FUNCTION__, __LINE__, __FILE__, MOCK_RETV,  \
                             count);                                                              \
    } while (0);
